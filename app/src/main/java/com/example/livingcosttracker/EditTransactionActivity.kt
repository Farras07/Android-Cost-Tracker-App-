    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnCancel.setOnClickListener {
            if (transactionId != -1) {
                lifecycleScope.launch {
                    try {
                        db.cashflowDao().deleteCashflowById(transactionId)
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@EditTransactionActivity, "Successfully Deleted!", Toast.LENGTH_SHORT).show()
                            finish()
                        }
                    } catch (e: Exception) {
                        Log.e("EditTransactionActivity", "Failed to Delete Transaction: ${e.message}", e)
                        withContext(Dispatchers.Main) {
                            Toast.makeText(this@EditTransactionActivity, "Failed to Delete Transaction: ${e.message}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
            } else {
